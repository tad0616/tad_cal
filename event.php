...
function insert_tad_cal_event()
{
    ...
    if ('1' == $allDay) {
        ...
        $rrule_start = mb_substr(str_replace(':', '', str_replace('-', '', date('c', strtotime($_POST['start'])))), 0, 15);
        $rrule_end   = mb_substr(str_replace(':', '', str_replace('-', '', date('c', strtotime($_POST['end'])))), 0, 15);
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
    Utility::query($sql, 'sssssssssiiisss', [$title, $start, $end, $recurrence, $location, $kind, $details, $etag, $id, $sequence, $uid, (int) $cate_sn, (int) $allDay, $tag, $last_update]) or Utility::web_error($sql, __FILE__, __LINE__);
