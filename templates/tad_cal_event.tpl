...
<{if $op=="show_one_tad_cal_event"}>
  ...
  <div style='text-align:right;font-size:12px;color:gray;'><{$cate_title}> / Posted by <{$uid_name}></div>
...
<{elseif $op=="list_tad_cal_event"}>
  ...
  <{foreach from=$all_content item=cal}>
    <tr>
        <td style='font-size:12px;font-family:arial;'><{$cal.date}></td>
        <td><a href='<{$xoops_url}>/modules/tad_cal/event.php?sn=<{$cal.sn}>'><{$cal.re}><{$cal.title}></a><div style='color:gray;font-size:12px;'><{$cal.details}></div></td>
        <td><{$cal.location}></td>
...
<{else}>
  ...
  <form action="<{$xoops_url}>/modules/tad_cal/event.php" method="post" id="myForm" enctype="multipart/form-data" class="form-horizontal" role="form">
    ...
    <div class="form-group">
      ...
      <div class="col-sm-10">
        <input type="text" id="title" name="title" value="<{$title}>" id="title" class="validate[required] form-control">
      </div>
    </div>
    ...
    <div class="form-group">
      ...
      <!--開始時間-->
      <div class="col-sm-2">
        <input type="text" id="start" name="start" value="<{$start_allday}>" id="start_allday" class="validate[required] form-control" onClick="WdatePicker({dateFmt:'yyyy-MM-dd' , startDate:'%y-%M-%d'})" onChange="check_end();month_repeat_chk();">
      </div>

      <!--結束時間-->
      <div class="col-sm-2">
        <input type="text" id="end" name="end" value="<{$end_allday}>" id="end_allday" onClick="WdatePicker({dateFmt:'yyyy-MM-dd' , startDate:'%y-%M-%d'})" onChange="update_long();" class="form-control">
      </div>
      <div class="col-sm-6">
        <!--全日事件-->
        ...
        <!--重複事件選項-->
        <label class="checkbox-inline">
          <input type="checkbox" name="repeat" id="repeat_checkbox" value="1" <{$repeat_1}>>
          <{$smarty.const._MD_TADCAL_RECURRENCE}>
        </label>
      </div>
    </div>
        ...
        <div class="col-sm-7">
          ...
          <div id="month_repeat" class="form-group">
            ...
            <div class="col-sm-9">
              ...
              <label class="radio-inline">
                <input type="radio" name="month_repeat" value="BYDAY" id="BYDAY"><span id="get_w"></span>
              </label>
            </div>
          </div>
        </div>
