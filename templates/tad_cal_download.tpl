...
<form action="word.php" method="post" id="myForm" class="form-horizontal" role="form">
  ...
  <div class="form-group row">
    ...
    <!--開始時間-->
    <div class="col-sm-2">
      <input type="text" name="start" title="start" value="<{$start}>" id="start_allday" class="validate[required] form-control" onClick="WdatePicker({dateFmt:'yyyy-MM-dd' , startDate:'%y-%M-%d'})">
    </div>
    <!--結束時間-->
    <div class="col-sm-2">
      <input type="text" name="end" title="end" value="<{$end}>" id="end_allday" onClick="WdatePicker({dateFmt:'yyyy-MM-dd' , startDate:'%y-%M-%d'})"  class="form-control">
    </div>
    <!--列出類型-->
    <label class="col-sm-2 col-form-label text-sm-right control-label">
